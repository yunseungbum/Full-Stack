...
public class Ex_Variable01 {
	public static void main(String[] args) {
		...
		//변수명 명명 규칙
		//사용 용도를 최대한 명확하게 보여줄 수 있는 이름이라면 개발자가 마음대로 지정할 수 있지만 다음과 같은 규칙을 지켜야한다.
		
		//1. 숫자가 먼저 들어가면 안된다.
		//2. 영어 대소문자를 구별한다.(Name 과 name은 다른 변수라고 인식)
		//3. 일반적으로 영어 소문자로 시작한다.
		//4. _,$,이외의 특수문자는 사용할 수 없다.
		//5. 예약어 금지(if,for,while,switch 등등)
		//6. 한글은 쓰지 말것
		
		//변수명은 문자 수의 제한이 없으므로 최대한 변수의 의미를 쉽게 파악할 수 있도록 구체적으로 명명할것.
		...
		//스네이크 표기법(snake case)
		//모든 단어가 소문자로 시작하고, 단어와 단어 사이트_로 연결하는 방식
		...
		//= 기호를 사용
		//수학에서는 좌변과 우변이 같다는 의미이지만 프로그램에서는 우변에 값을 좌변에 대입한다는 의미
		
		int studentAge; // 학생의 나이를 저장할 수 있는 변수의 선언
		studentAge = 20;//변수에 값을 대입
		System.out.println(studentAge);
		
		//변수를 선언하고 처음으로 값을 대입하는것을'초기화'(initialization)라고 한다. 초기값을 지정한다 라는 의미
